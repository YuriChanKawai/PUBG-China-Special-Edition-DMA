...
static int monitor_packet_count = 0;        // 接收到的包数量
...
int kmNet_monitor_get_buttons()
{
	return hw_mouse.buttons;
}
...
int kmNet_init(char* ip, char* port, char* mac)
{
	...
	// 设置非阻塞模式
	unsigned long ul = 1;
	ioctlsocket(sockClientfd, FIONBIO, &ul);
    ...
    for (int i = 0; i < max_retries; i++) {
        ...
        fd_set fdread;
        FD_ZERO(&fdread);
        FD_SET(sockClientfd, &fdread);
        struct timeval tv;
        tv.tv_sec = 0;
        tv.tv_usec = 200000; // 200ms 等待

        int select_ret = select(0, &fdread, NULL, NULL, &tv);
        if (select_ret > 0) {
            received = 1;
            break;
        }
    ...
    if (received) {
        ...
        sockClientfd = -1;
        return err_net_timeout; // 自定义超时错误
    }
}
...
// 加密版使能键鼠监控
int kmNet_enc_monitor(short port)
{
	int err; client_tx  tx_enc = { 0 };
	if (sockClientfd <= 0)		return err_creat_socket;
	WaitForSingleObject(m_hMutex_lock, 2000);
...
int kmNet_monitor_get_packet_count()
{
	return monitor_packet_count;
}
